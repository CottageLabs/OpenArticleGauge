...
from openarticlegauge.plugins.bmc import BMCPlugin as MyPlugin

# a list of urls which your plugin should be able to support
SUPPORTED_URLS = ["http://www.biomedcentral.com/983242",
                  "http://www.biomedcentral.com/1471-2164/13/425",
                  "http://www.biomedcentral.com/1471-2458/13/1092",
                  "http://www.ijbnpa.org/content/11/1/34"]
...
UNSUPPORTED_URLS = ["springer.com/article", "sciencedirect.com/article",
                    "http://www.plosone.org/", "askjdfsakjdhfsa"]
...
RESOURCE_AND_RESULT = {
    os.path.join(os.path.dirname(os.path.abspath(__file__)),
                 "resources", "bmc_10.1186_1471_1471-2164-13-425.html"):
        {
            "id": None,  # there should be no id field
            "version": "2.0",  # version should be the empty string
            "type": "cc-by",
            "jurisdiction": "",
            # jurisdiction should be the empty string
            "open_access": True,
            "BY": True,
            "NC": False,
            "ND": False,
            ...
            "provenance": {
                "handler": 'bmc',
                # name of plugin which processed this record
                "handler_version": '0.1',
                # version of plugin which processed this record
                "category": "page_scrape",  # category is page_scrape
                "description": 'License decided by scraping the resource at http://www.biomedcentral.com/1471-2164/13/425 and looking for the following license statement: "This is an Open Access article distributed under the terms of the Creative Commons Attribution License (<a href=\'http://creativecommons.org/licenses/by/2.0\'>http://creativecommons.org/licenses/by/2.0</a>), which permits unrestricted use, distribution, and reproduction in any medium, provided the original work is properly cited.".',

                # description is a long string
                "agent": config.agent,  # agent is from configuration
                "source": "http://www.biomedcentral.com/1471-2164/13"
                          "/425",
                # source is the url where we look this record up
                "date": -1
                # date is not null (but we don't know the exact value)
            }
        },
    os.path.join(os.path.dirname(os.path.abspath(__file__)),
                 "resources", "bmc_10.1186_1471-2458-13-1092.html"):
        {
            "id": None,  # there should be no id field
            "version": "2.0",  # version should be the empty string
            "type": "cc-by",
            ...
            "provenance": {
                ...
                # version of plugin which processed this record
                "category": "page_scrape",  # category is page_scrape
                "description": 'License decided by scraping the resource at http://www.biomedcentral.com/1471-2458/13/1092 and looking for the following license statement: "This is an Open Access article distributed under the terms of the Creative Commons Attribution License (<a href=\'http://creativecommons.org/licenses/by/2.0\'> http://creativecommons.org/licenses/by/2.0</a>), which permits unrestricted use, distribution, and reproduction in any medium, provided the original work is properly cited.".',
                # description is a long string
                "agent": config.agent,  # agent is from configuration
                "source": "http://www.biomedcentral.com/1471-2458/13/1092",
                # source is the url where we look this record up
                "date": -1
                # date is not null (but we don't know the exact value)
            }
    ...
    os.path.join(os.path.dirname(os.path.abspath(__file__)),
                 "resources", "bmc_10.1186_1479-5868-11-34.html"):
        {
            "id": None,  # there should be no id field
            "version": "2.0",  # version should be the empty string
            "type": "cc-by",
            ...
            "provenance": {
                ...
                # version of plugin which processed this record
                "category": "page_scrape",  # category is page_scrape
                "description": 'License decided by scraping the resource at http://www.ijbnpa.org/content/11/1/34 and looking for the following license statement: "This is an Open Access article distributed under the terms of the Creative Commons Attribution License (<a href=\'http://creativecommons.org/licenses/by/2.0\'>http://creativecommons.org/licenses/by/2.0</a>), which permits unrestricted use, distribution, and reproduction in any medium, provided the original work is properly credited.".',
                # description is a long string
                "agent": config.agent,  # agent is from configuration
                "source": "http://www.ijbnpa.org/content/11/1/34",
                # source is the url where we look this record up
                "date": -1
                # date is not null (but we don't know the exact value)
            }
...
class TestProvider(TestCase):
    ...
    def test_02_supports_fail(self):
        ...
        for url in UNSUPPORTED_URLS:
            assert not p.supports(
                {"url": [url]}), "Failed with URL " + url
    ...
    def __run_resource_and_result_test(self):
        ...
        for path, comparison in RESOURCE_AND_RESULT.iteritems():
            ...
            assert "license" in record[
                'bibjson'], 'While testing with ' + path
            ...
            for key, value in comparison.iteritems():
                ...
                if value is None:
                    ...
                    assert key not in licence or licence.get(
                        key) == "", ((key, value), licence.get(key))
                elif value == -1:
                    # the resulting object MUST have the key
                    assert key in licence, (
                        (key, value), licence.get(key))
                else:
                    # the resulting object must match the comparison object
                    assert value == licence.get(key), (
                        (key, value), licence.get(key))
            ...
            for key, value in comparison.get("provenance",
                ...
                if value is None:
                    ...
                    assert key not in prov or prov.get(key) == "", (
                        (key, value), prov.get(key))
                elif value == -1:
                    # the resulting object MUST have the key
                    assert key in prov, ((key, value), prov.get(key))
                else:
                    # the resulting object must match the comparison object
                    assert value == prov.get(key), (
                        (key, value), prov.get(key))
