...
def batchtest():
    ...
    r = requests.get('http://92.235.188.179:9200/occ/record/_search?q=journal.name:plos* OR journal.name:bmc*&from=50&size=10000')
    ...
    for _id in ids:
        ...
        if len(_id) < 9:
            ...
            sub = 'doi:'
        rr = requests.get('http://test.cottagelabs.com:9200/oag/record/' + sub + _id.replace('/','_'))
        rs = rr.status_code
        #print _id, rs
        if rs != 200:
            err += 1
