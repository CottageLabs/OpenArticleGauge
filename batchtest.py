...
def batchtest():
    ...
    r = requests.get('http://129.67.24.26:9200/test/record/_search?q=journal.name:plos* OR journal.name:bmc*&size=5')
    ...
    while len(ids):
        ...
        rr = requests.post('http://iioa.cottagelabs.com/lookup/',data=json.dumps(ids), headers=headers)
        rs = rr.json()
